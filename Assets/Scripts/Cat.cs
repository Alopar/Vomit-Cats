...
namespace VomitCats
{
    public class Cat : MonoBehaviour, IDrawSort, IPointerClickHandler, IPointerEnterHandler
    {
        ...
        public void OnPointerEnter(PointerEventData eventData)
        {
            switch (GameSettings.CurrentLanguage)
            {
                case Language.Russian:
                    GameUiController.Instance.SetDescription(_ruName);
                    break;
                case Language.English:
                    GameUiController.Instance.SetDescription(_enName);
                    break;
            }
        }
        #endregion
        ...
        public void SetDrawOrder(int order)
        {
            if (_isOnHand)
            {
                ...
                if (_cleaner.VerticalOrientation == VerticalOrientation.Up)
                {
                    _spriteRenderer.sortingOrder = _cleaner.DrawSortOrder - 1;
                }
                else
                {
                    _spriteRenderer.sortingOrder = _cleaner.DrawSortOrder + 1;
                ...
                if(_cleaner.HorizontalOrientation == HorizontalOrientation.Right && _cleaner.VerticalOrientation == VerticalOrientation.Up)
                {
                    direction.x = 1f;
                    direction.y = -1f;
                }

                if (_cleaner.HorizontalOrientation == HorizontalOrientation.Right && _cleaner.VerticalOrientation == VerticalOrientation.Bottom)
                {
                    direction.x = -1f;
                    direction.y = -1f;
                }

                if (_cleaner.HorizontalOrientation == HorizontalOrientation.Left && _cleaner.VerticalOrientation == VerticalOrientation.Bottom)
                {
                    direction.x = -1f;
                    direction.y = 1f;
                }

                if (_cleaner.HorizontalOrientation == HorizontalOrientation.Left && _cleaner.VerticalOrientation == VerticalOrientation.Up)
                {
                    direction.x = 1f;
                    direction.y = 1f;
                }
